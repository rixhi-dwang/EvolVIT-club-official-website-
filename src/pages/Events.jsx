        ...
        <div className="grid md:grid-cols-2 gap-8">
          {events.map((event, index) => (
            <ScrollReveal key={event.id} direction="up" delay={index * 0.1}>
              <GlowingCard gradient={event.gradient}>
                <Link to={`/events/${event.id}`} className="block p-8">
                  <div className="relative">
                    ...
                    {event.status === 'upcoming' && (
                      <div className="mt-8 w-full py-3 px-6 rounded-lg border border-[#00f5ff]/30 text-[#00f5ff] text-center hover:bg-[#00f5ff] hover:text-[#05060a] transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,245,255,0.5)] font-semibold">
                        Register Now →
                      </div>
                    )}
                  </div>
